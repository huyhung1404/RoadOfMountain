...
namespace TTT
{
    public class TerrainTools : EditorWindow
    {
        ...
        bool
            assignNormalMapsIfFounded =
                true; // checks if texture has normals map in same folder: texturename.png & texturename_normal.png
        ...
        System.Diagnostics.Stopwatch
            stopwatch = new System.Diagnostics.Stopwatch(); // timer for timing method durations


        bool
            timeStampFileNames =
                false; // if enabled, every time you generate, new terrain is saved with a new name (as new terrain)
        ...
        void DrawAutoMagicGUI()
        {
            ...
            if (toggleAutoMagic)
            {
                ...
                GUILayout.BeginHorizontal();
                splatMap = EditorGUILayout.ObjectField(
                    new GUIContent("Splatmap Object", "Heightmap, Splatmap, Treemap, Grassmap"), splatMap,
                    typeof(Object), false);
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                EditorGUILayout.PrefixLabel("Textures");
                GUILayout.FlexibleSpace();
                splatTextures[0] = (Texture2D) EditorGUILayout.ObjectField(splatTextures[0], typeof(Texture2D), false,
                    GUILayout.Height(48));
                splatTextures[1] = (Texture2D) EditorGUILayout.ObjectField(splatTextures[1], typeof(Texture2D), false,
                    GUILayout.Height(48));
                splatTextures[2] = (Texture2D) EditorGUILayout.ObjectField(splatTextures[2], typeof(Texture2D), false,
                    GUILayout.Height(48));
                splatTextures[3] = (Texture2D) EditorGUILayout.ObjectField(splatTextures[3], typeof(Texture2D), false,
                    GUILayout.Height(48));
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                ...
                GUILayout.FlexibleSpace();
                grassTextures[0] = (Texture2D) EditorGUILayout.ObjectField(grassTextures[0], typeof(Texture2D), false,
                    GUILayout.Height(32));
                grassTextures[1] = (Texture2D) EditorGUILayout.ObjectField(grassTextures[1], typeof(Texture2D), false,
                    GUILayout.Height(32));
                grassTextures[2] = (Texture2D) EditorGUILayout.ObjectField(grassTextures[2], typeof(Texture2D), false,
                    GUILayout.Height(32));
                GUILayout.EndHorizontal();
                EditorGUILayout.Separator();
                ...
                GUILayout.FlexibleSpace();
                detailObjects[0] =
                    (GameObject) EditorGUILayout.ObjectField(detailObjects[0], typeof(GameObject), false);
                detailObjects[1] =
                    (GameObject) EditorGUILayout.ObjectField(detailObjects[1], typeof(GameObject), false);
                detailObjects[2] =
                    (GameObject) EditorGUILayout.ObjectField(detailObjects[2], typeof(GameObject), false);
                GUILayout.EndHorizontal();
                EditorGUILayout.Separator();
                ...
                if (toggleAdvanced)
                {
                    ...
                    GUI.changed = false;
                    selectedTexturePack =
                        EditorGUILayout.Popup("Texture pack", selectedTexturePack, texturePackOptionTitles);
                    if (GUI.changed)
                    {
                    ...
                    // misc options
                    assignNormalMapsIfFounded =
                        EditorGUILayout.Toggle("Use normal maps if founded", assignNormalMapsIfFounded);
                }
            }
        ...
        public void FixTextureSettings(Texture2D texture)
        {
            if (texture == null)
            {
                Debug.LogError("FixFormat failed - Texture is null");
                return;
            }
            ...
            {
                Debug.LogError("FixFormat failed - Texture path is null");
                return;
            }
...
//				Debug.Log(filename);
        ...
        void AutoMagic()
        {
            if (splatMap == null)
            {
                Debug.LogError("No splatmap assigned");
                return;
            }

            if (!ValidateAutomagicTextures())
            {
                Debug.LogError("Not all textures are assigned");
                return;
            }

            if (!ValidateAutomagicTrees())
            {
                Debug.LogError("Not all trees are assigned");
                return;
            }
            ...
            {
                Debug.LogError("Filenames must start with \"number\" and \"-\" character. Example: 1-splatmap.tif");
                return;
            }
            ...
            if (!File.Exists(splatmap_name))
            {
                Debug.LogError("Splatmap file not found:" + splatmap_name +
                               " \n Make sure that your filenames start with \"number\" and \"-\" character. Example: 1-splatmap.tif");
                return;
            }
            ...
            myTerrain.terrainData.SetHeights(0, 0,
                ReadRawHeightMap(heightmap_name, splatA.width + 1, myTerrain.terrainData.heightmapResolution,
                    myTerrain.terrainData.heightmapResolution));
        ...
        void AutoMagicDefaults()
        {
            ...
            switch (selectedTexturePack)
            {
                ...
                case 0:
                    ...
                    if (File.Exists(tempFilePath + tempFileName))
                        splatTextures[0] =
                            (Texture2D) AssetDatabase.LoadAssetAtPath(tempFilePath + tempFileName, typeof(Texture2D));
            ...
            if (File.Exists(tempFilePath + tempFileName))
                treeObjects[0] =
                    (GameObject) AssetDatabase.LoadAssetAtPath(tempFilePath + tempFileName, typeof(GameObject));
        ...
        {
            if (splatTextures[0] == null || splatTextures[1] == null || splatTextures[2] == null ||
                splatTextures[3] == null) return false;
            return true;
        }
        ...
        void SetupTextureNormals()
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    string tempNormalMapFile = Path.GetDirectoryName(tempFileName) + Path.DirectorySeparatorChar +
                                               tempFileWithoutExtension + "_normal" + Path.GetExtension(tempFileName);
                    if (File.Exists(tempNormalMapFile))
                        splatTexNormals[i] =
                            (Texture2D) AssetDatabase.LoadAssetAtPath(tempNormalMapFile, typeof(Texture2D));
                }
            }
        }
        ...
        bool CheckSplatmap()
        {
            ...
            {
                EditorUtility.DisplayDialog("Wrong size",
                    "First splatmap must be square (width and height must be the same)", "Cancel");
                return false;
            }
            ...
            if (splatB != null)
            {
                ...
                {
                    EditorUtility.DisplayDialog("Wrong size",
                        "Second splatmap must be square (width and height must be the same)", "Cancel");
                    return false;
                }

                if (Mathf.ClosestPowerOfTwo(splatB.width) != splatB.width)
                {
                    EditorUtility.DisplayDialog("Wrong size", "Second splatmap width and height must be a power of two",
                        "Cancel");
                    return false;
                }
            }
...
#pragma warning restore 618
        ...
        void ApplySplatmap()
        {
            ...
            if (terrainData == null)
            {
                Debug.LogError("Failed getting terrain data");
                return;
            }

            if (terrainData.alphamapLayers < 4)
            {
                EditorUtility.DisplayDialog("Missing Terrain Textures",
                    "Please set up at least 4 textures in the terrain painter dialog", "Cancel");
                return;
            }
            ...
            {
                EditorUtility.DisplayDialog("Missing Terrain Textures",
                    "Please set up at least 8 textures in the terrain painter dialog", "Cancel");
                return;
            }
            ...
            for (int y = 0; y < width; y++)
            {
                ...
                {
                    if (EditorUtility.DisplayCancelableProgressBar("Applying splatmap", "calculating...",
                        Mathf.InverseLerp(0.0f, (float) width, (float) y)))
                    {
...
//							splatmapData[x,y,4] = 0;
        ...
        void ApplyTreemap()
        {
            ...
            if (terrainData.treePrototypes.Length < 3)
            {
                ...
                {
                    Debug.LogWarning(
                        "You should assign 3 trees to Terrain to use all 3 splatmap color channels (at Place Trees tab)");
                }
            }
            ...
            for (float y = 1; y < width - 1; y += Step)
            {
                ...
                {
                    progress = 0;
                    if (EditorUtility.DisplayCancelableProgressBar("Placing trees", "placed " + trees + " trees so far",
                        Mathf.InverseLerp(0.0f, (float) width, (float) y)))
                    {
                        EditorUtility.ClearProgressBar();
                        return;
        ...
        bool CheckGrassmap()
        {
            if (grassmap != null)
            {
                ...
                {
                    EditorUtility.DisplayDialog("Wrong size", "grassmap width and height must be a power of two",
                        "Cancel");
                    return false;
                }
            }
            ...
            if (bushmap != null)
            {
                ...
                if (Mathf.ClosestPowerOfTwo(w) != w)
                {
                    EditorUtility.DisplayDialog("Wrong size", "bushmap width and height must be a power of two",
                        "Cancel");
                    return false;
                }
            }
        ...
        void ApplyGrassmap()
        {
            ...
            if (grassmap != null)
            {
                if (SetDetailmap(grassmap, grassDensity, 0, grassclumping, "Grass map"))
                    Debug.Log("Grass map applied " + GetTimerTime());
            }

            if (bushmap != null)
            {
                //if (SetDetailmap(bushmap, bushmod, 3, 0.0f, "Bush map")) Debug.Log("Bush map applied.");
                if (SetDetailmap(bushmap, bushDensity, grassmap == null ? 0 : 3, 0.0f, "Bush map"))
                    Debug.Log("Bush map applied " + GetTimerTime());
            }
        ...
        bool SetDetailmap(Texture2D map, float mod, int firstlayer, float clumping, string MapName)
        {
            ...
            if (terrainData.detailPrototypes.Length < 3)
            {
                ...
                {
                    Debug.LogError(
                        "You need to add at least 1 detail textures or 1 detail meshes to Terrain (at Paint Details)");
                    return false;
                }

                Debug.LogWarning(
                    "You should add 3 detail textures or 3 detail meshes to Terrain to use all splat map color channels (at Paint Details)");
            }
            ...
            for (int y = 0; y < res; y++)
            {
                if (EditorUtility.DisplayCancelableProgressBar("Applying " + MapName, "Calculating...",
                    Mathf.InverseLerp(0.0f, (float) res, (float) y)))
                {
                    EditorUtility.ClearProgressBar();
                    return false;
                }
        ...
        bool CheckOverlaymap()
        {
            if (overlayMap == null)
            {
                Debug.LogError("No overlay map assigned");
                return false;
            }
            ...
            {
                EditorUtility.DisplayDialog("Wrong size", "OverlayMap width and height must be a power of two",
                    "Cancel");
                return false;
            }
            ...
            {
                EditorUtility.DisplayDialog("Wrong size",
                    "OverlayMap must have same size as existing splatmap (" + myTerrain.terrainData.alphamapResolution +
                    ")", "Cancel");
                return false;
            }
        ...
        void ApplyOverlaymap()
        {
            ...
            for (int y = 0; y < w; y++)
            {
                if (y % 10 == 0)
                {
                    if (clearTrees)
                    {
                        if (EditorUtility.DisplayCancelableProgressBar("Overlay map",
                            "updating terrain and trees (" + TreesRemoved + " trees removed)",
                            Mathf.InverseLerp(0.0f, (float) w, (float) y)))
                        {
                            EditorUtility.ClearProgressBar();
                            return;
                        }
                    ...
                    {
                        if (EditorUtility.DisplayCancelableProgressBar("Overlay map", "updating terrain",
                            Mathf.InverseLerp(0.0f, (float) w, (float) y)))
                        {
                            EditorUtility.ClearProgressBar();
                            return;
...
//					float value = OverlayMapColors[((w-1)-x)*w + y].grayscale;
                    ...
                    if (value > overlayThreshold)
                    {
                        ...
                        if (changeTerrain > 0.01f || changeTerrain < -0.01f)
                        {
                            if (value > overlayThreshold)
                            {
                                ...
                                for (int a = 0; a < terrainSample; a++)
                                for (int b = 0; b < terrainSample; b++)
                                {
                        ...
                        if (clearTrees)
                        {
                            for (int i = NewTrees.Count - 1; i >= 0; i--)
                            {
                                ...
                                float distance = Vector2.Distance(
                                    new Vector2(MyTree.position.z * w, MyTree.position.x * w),
            ...
            if (clearGrass)
            {
                ...
                {
                    if (EditorUtility.DisplayCancelableProgressBar("Overlay map", "clearing away grass",
                        Mathf.InverseLerp(0.0f, (float) l, (float) LayerCount)))
                    {
                        EditorUtility.ClearProgressBar();
                        return;
                    }
...
//			int heightmapHeight = MyTerrain.terrainData.heightmapHeight;
            ...
            for (int i = 0; i < heightmapHeight; i++)
            {
                if (i % 10 == 0)
                {
                    if (EditorUtility.DisplayCancelableProgressBar("Importing heightmap", "calculating...",
                        Mathf.InverseLerp(0.0f, (float) heightmapHeight, (float) i)))
                    {
                        EditorUtility.ClearProgressBar();
                        return null;
                    }
